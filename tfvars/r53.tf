resource "aws_route53_record" "expense" {

    for_each = aws_instance.expense
    zone_id = var.zone_id
    name  = each.key == "frontend-prod" ? var.domain_name : "${each.key}.${var.domain_name}"
    ## if it is "frontend-prod" --> use pspkdevops.online and for all others append domain name
    type    = "A"
    ttl     = 1
    records = startswith(each.key, "frontend") ? [each.value.public_ip] : [each.value.private_ip]
    ## for route53 if it is frontend then we need to user public_ip and for others it is private_ip
    allow_overwrite = true    # it will override the existing route53 records if any
}